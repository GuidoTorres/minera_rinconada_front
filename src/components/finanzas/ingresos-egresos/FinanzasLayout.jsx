...
const Finanzas = () => {
  ...
  const formatSaldoMensual = () => {
    ...
    console.log(saldoMensual);
    const prueba = saldoMensual?.reduce((acc, curr) => {
      const findData = acc.find(
        (ele) => ele.movimiento === curr.movimiento && ele.fecha === curr.fecha
      );

      if (findData) {
        findData.ingresos =
          parseFloat(findData.ingresos) + parseFloat(curr.ingresos);
        findData.egresos = parseFloat(findData.egresos) + parseFloat(curr.egresos);
      } else {
        acc.push(curr);
      }
    ...
    const prueba2 = prueba?.reduce((acc, curr) => {
      const sumData = acc.find((ele) => ele.fecha === curr.fecha);

      if (sumData) {

        sumData.total = parseFloat(curr.ingresos) - parseFloat(curr.egresos);
        acc.push(sumData);
      } else {
        curr.total = curr.egresos || curr.ingresos;
        acc.push(curr);
      }
    ...
    const prueba3 = [...new Set(prueba2)];

    const prueba4 = prueba3?.map((item) => {
      return {
        total: item.total,
        mes:
          item.fecha === 1
